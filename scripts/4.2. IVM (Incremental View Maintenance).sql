/* 4. IVM (Incremental View Maintenance).sql */
--BEGIN TRANSACTION;
WITH
...
top_craftsman AS (-- идентификатор самого популярного мастера ручной работы у заказчика
    SELECT 
            t.customer_id AS customer_id_top,
            t.craftsman_id AS top_craftsman_id
    FROM
        (SELECT
            fo.customer_id,
            fo.craftsman_id,
            ROW_NUMBER() OVER (PARTITION BY fo.customer_id ORDER BY COUNT(*) DESC) rn
        FROM 
            dwh.f_order fo
        GROUP BY
            fo.customer_id,
...
dwh_delta_insert_result AS ( -- делаем расчёт витрины по новым данным. Этой информации по покупателям в рамках расчётного периода раньше не было, это новые данные. Их можно просто вставить (insert) в витрину без обновления
    SELECT  
            ...
            T4.top_craftsman_id AS top_craftsman_id, ----------------------------------------!!!!!!!!!!!!!!!!!!!!!!!!!! необходим запрос
            ...
            FROM (
                SELECT     -- в этой выборке объединяем две внутренние выборки по расчёту столбцов витрины и применяем оконную функцию для определения самой популярной категории товаров
                        *,
                        ...
                        FROM ( 
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбца с самой популярной категорией товаров у покупателя. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним JOIN
                               ...
                               INNER JOIN top_craftsman AS T5 ON T2.customer_id = T5.customer_id_top   ---???
...
update_delta AS ( -- выполняем обновление показателей в отчёте по уже существующим покупателям
